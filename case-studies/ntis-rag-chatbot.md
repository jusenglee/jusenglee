...
이 프로젝트에서 가장 큰 설계 전환은 질의를 하나의 retrieval(벡터 DB 검색) 규칙으로 처리하지 않고, **SEARCH / LOOKUP / JOIN** 세 가지 모드로 분리한 것입니다.
...
## 3.2. Planner(검색전략 선정)–Contract(검색전략 검증)–Executor(검색전략 실행) 구조 도입
...
## 3.4. People(사람) / Org(기관) 질의는 1급 엔티티로 승격
...
- Strategy(검색 플래너 결과) 정합성 검증
- ID(데이터 고유 ID) / NO(데이터 그룹 ID) 혼합 입력 검출
...
- Triton 모델 응답 속도: **평균 3.4초**
- 전체 파이프라인 처리시간: **약 3.8초 / 질의**
